...
function ProfilePageInner() {
    ...
    /* ---- Skill metrics from localStorage ---- */
    ...
    // Direct null checks for all user data as specified in requirement
    const displayName = studentName || user?.displayName || 'Pilot';
    const avatarUrl = profileImageUrl || user?.photoURL || '/default-avatar.png';
    const finalCollegeName = collegeName || (user as any)?.collegeName || 'Not specified';
    const finalSkillScore = skillScore || (user as any)?.stats?.score || 0;
    const finalLabsCompleted = labsCompleted || 0;
    const finalStreakDays = streakDays || 0;
    ...
    try {
                ...
                <div className="max-w-5xl mx-auto px-6 py-10">
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-4">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    {(yearOfStudy || finalCollegeName) ? (
                                        <p className="text-xs mt-1 font-bold" style={{ color: 'var(--text-tertiary)' }}>
                                            {yearOfStudy ? `Year ${yearOfStudy}` : ''}{yearOfStudy && finalCollegeName && finalCollegeName !== 'Not specified' ? ' • ' : ''}{finalCollegeName}
                                        </p>
                                    ) : (
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        <div className="skeu-card p-5 text-center">
                            <p className="font-black text-3xl flex items-center justify-center h-[36px]" style={{ fontFamily: 'var(--font-display)', color: 'var(--accent-primary)' }}>
                                {finalSkillScore ? finalSkillScore : <span className="text-xs font-semibold" style={{ color: 'var(--text-tertiary)' }}>No data yet</span>}
                            </p>
                            <p className="text-[10px] font-bold uppercase tracking-widest mt-1" style={{ color: 'var(--text-tertiary)' }}>AI Skill Score</p>
                        </div>
                        <div className="skeu-card p-5 text-center">
                            <p className="font-black text-3xl flex items-center justify-center h-[36px]" style={{ fontFamily: 'var(--font-display)', color: '#6366f1' }}>
                                {finalLabsCompleted ? finalLabsCompleted : <span className="text-xs font-semibold" style={{ color: 'var(--text-tertiary)' }}>No data yet</span>}
                            </p>
                            <p className="text-[10px] font-bold uppercase tracking-widest mt-1" style={{ color: 'var(--text-tertiary)' }}>Labs Completed</p>
                        </div>
                        <div className="skeu-card p-5 text-center">
                            <p className="font-black text-3xl flex items-center justify-center h-[36px]" style={{ fontFamily: 'var(--font-display)', color: '#F59E0B' }}>
                                {finalStreakDays ? (
                                    <>
                                        {finalStreakDays} <span className="text-xl ml-1">🔥</span>
                                    </>
                                ) : (
                                    <span className="text-xs font-semibold" style={{ color: 'var(--text-tertiary)' }}>No data yet</span>
                                )}
                            </p>
                        ...
                        <div className="skeu-card p-5 text-center">
                            <p className="font-black text-xl flex items-center justify-center h-[36px]" style={{ fontFamily: 'var(--font-display)', color: '#10B981' }}>
                                {courseName || <span style={{ color: 'var(--text-tertiary)', fontSize: '14px' }}>No path yet</span>}
                            </p>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mb-8 skeu-card p-6">
                        ...
                        <div className="flex gap-4 flex-wrap">
                            ...
                            {finalLabsCompleted <= 10 && finalStreakDays <= 3 && (
                                ...
                                {recentActivity.map((activity, i) => (
                                    <div key={i} className="skeu-card p-6 border-l-4" style={{ borderLeftColor: 'var(--accent-primary)' }}>
                                        <div className="flex items-center justify-between mb-3">
                                            <h3 className="font-bold text-sm truncate pr-2">{String(activity || '').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h3>
                                ...
                                <div className="flex flex-col md:flex-row gap-6">
                                    ...
                                    <div className="flex flex-col items-center text-center md:w-1/3 border-b md:border-b-0 md:border-r border-gray-700/30 pb-6 md:pb-0 md:pr-6">
                                        ...
                                        <div className="flex gap-4 mb-4 text-xs">
                                            <div><span className="font-bold">{githubData?.public_repos ?? 0}</span> Repos</div>
                                            <div><span className="font-bold">{githubData?.followers ?? 0}</span> Followers</div>
                                            <div><span className="font-bold">{githubData?.following ?? 0}</span> Following</div>
                                        </div>
                                        <button onClick={() => window.open(githubData?.html_url || `https://github.com/${githubUsername}`, '_blank')} className="clay-btn clay-btn-secondary clay-btn-sm w-full">
                                            View Profile
                                        </button>
                                    </div>
                                        ...
                                        {githubRepos.length > 0 ? (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {githubRepos.map(repo => (
                                                    <div key={repo?.id || repo?.name} className="p-4 rounded-xl" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-medium)' }}>
        ...
        console.error('[ProfilePageInner] Render Crash Error Logged:', renderError);
        throw renderError; // Re-throw to propagate to ErrorBoundary
    }
}

/* ------------------------------------------------------------------ */
/*  Default export wraps inner page with error boundary                */
/* ------------------------------------------------------------------ */
