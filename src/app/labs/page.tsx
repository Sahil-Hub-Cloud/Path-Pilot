...
import { LABS, LAB_IDS_ORDERED } from '@/lib/data/labs';
import { FiTerminal, FiArrowRight, FiAward, FiStar, FiArrowLeft } from 'react-icons/fi';
        ...
        <header className="mb-12 text-center md:text-left flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-[#888899] text-sm md:text-base max-w-2xl">
              Test your skills with hands-on coding challenges. Complete labs to earn XP and showcase your technical abilities to employers.
            </p>
          </div>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {LAB_IDS_ORDERED.map(id => {
            const lab = LABS[id];
            if (!lab) return null;
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-auto pt-6 text-[10px] font-black text-gray-500 dark:text-[#555566]">
                  <span className={`px-2 py-1 rounded-md uppercase tracking-wider ${
                    lab.difficulty === 'Easy' ? 'text-green-700 bg-green-100 dark:bg-green-500/10 dark:text-green-400' :
                    lab.difficulty === 'Medium' ? 'text-orange-700 bg-orange-100 dark:bg-orange-500/10 dark:text-orange-400' :
                    'text-red-700 bg-red-100 dark:bg-red-500/10 dark:text-red-400'
                  }`}>
                    {lab.difficulty}
                  </span>
                  <span className="uppercase tracking-widest bg-white dark:bg-black/20 px-2 py-1 rounded-md border border-[#B48C5A]/10 dark:border-white/5">{lab.defaultLang}</span>
                </div>
